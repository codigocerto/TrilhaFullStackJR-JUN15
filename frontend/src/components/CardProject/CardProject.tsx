      ...
      <section>
        <ul className=" bg-slate-50 p-4 sm:px-8 sm:pt-6 sm:pb-8 lg:p-4 xl:px-8 xl:pt-6 xl:pb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 text-sm leading-6">
          {dadosTeste.map((projeto) => (
            <li key={projeto.id} className="rounded-md p-3 bg-white ring-1 ring-slate-200 shadow-sm hover:bg-blue-500 hover:ring-blue-500 hover:shadow-md">
              <a href={projeto.link} className="flex flex-col">
                <img src={projeto.img} alt={projeto.title} className="rounded-md mb-4" />
                <dl>
                  <div>
                    <dt className="sr-only">Título</dt>
                    <dd className="font-semibold text-slate-900 group-hover:text-white">{projeto.title}</dd>
                  </div>
                  <div>
                    <dt className="sr-only">Categoria</dt>
                    <dd className="text-blue-200 group-hover:text-white">{projeto.category}</dd>
                  </div>
                  <div className="mt-4">
                    <dt className="sr-only">Tags</dt>
                    <dd className="flex space-x-2">
                      {projeto.tags.map(tag => (
                        <span key={tag.id} className="bg-slate-200 text-slate-800 px-2 py-1 rounded">{tag.id}</span>
                      ))}
                    </dd>
                  </div>
                </dl>
